...
export class EmailService {
  ...
  async sendRejectionOutcome(request: VendorRequest): Promise<void> {
    ...
    const html = emailLayout(
      ...
      `<p style="margin:0 0 18px;font-size:16px;line-height:1.6;">Thank you for submitting the vendor registration application for <strong>${escapeHtml(request.vendorName)}</strong>.</p><p style="margin:0 0 18px;font-size:16px;line-height:1.6;">Our vendor management team has carefully reviewed the application. Unfortunately, we are unable to proceed with the registration at this time.</p><p style="margin:0 0 20px;font-size:16px;line-height:1.6;">You are welcome to submit a new vendor registration request at a later date.</p><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;"><tr><td style="width:38%;padding:10px;border:1px solid #e5e7eb;background:#f9fafb;font-weight:bold;">Request Number</td><td style="padding:10px;border:1px solid #e5e7eb;">#${escapeHtml(request.id)}</td></tr></table>`,
    )
