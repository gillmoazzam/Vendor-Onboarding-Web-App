...
export function RequestDetailsPage() {
  ...
  const decisionMutation = useMutation({
    mutationFn: async (action: 'approve' | 'reject') => (await api.post<{ status: RequestStatus; vendorId?: string }>(`/requests/${id}/decision`, { action })).data,
    onSuccess: (data) => {
  ...
  return <main className="mx-auto max-w-7xl space-y-6 p-6">
    ...
    <Card><CardHeader><CardTitle>Vendor Information</CardTitle></CardHeader><CardContent><dl className="grid gap-4 sm:grid-cols-2"><Detail label="Vendor / Company Name" value={request.vendorName} /><Detail label="Contact Person" value={request.contactPerson} /><Detail label="Email Address" value={request.contactEmail} /><Detail label="Reason for Consideration" value={request.reasonLabel} /><div className="sm:col-span-2"><Detail label="Vendor Legal Address" value={request.vendorAddress} /></div>{request.reasonOther && <div className="sm:col-span-2"><Detail label="Additional Reason Details" value={request.reasonOther} /></div>}</dl></CardContent></Card>
